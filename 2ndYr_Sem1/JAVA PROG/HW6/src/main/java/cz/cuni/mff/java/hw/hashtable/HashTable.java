...
public class HashTable implements IHashTable {
     ...
     */
    private void rehash() {
        DynamicArray newHashTable = new DynamicArray();
        for (int i = 0; i < hashTable.getCapacity() * 2; i++) {
            newHashTable.add(new SinglyLinkedList());
        }

        DynamicArray copyHashTable = new DynamicArray();
        for (int i = 0; i < hashTable.getCapacity(); i++) {
        ...
        for (int i = 0; i < copyHashTable.getCapacity(); i++) {
            ...
            for (Node entry : copyHashTable.get(i)) {
                int hashValue = Math.abs(entry.value.key.hashCode()) % hashTable.getCapacity();
                hashTable.get(hashValue).add(entry.value);
            }
        }
    }
