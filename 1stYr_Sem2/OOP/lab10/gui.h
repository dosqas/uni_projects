...
class AdoptionTableViewModel : public QAbstractTableModel
{
    Q_OBJECT
...
public:
    AdoptionTableViewModel(Service& serv, QObject* parent = nullptr) : QAbstractTableModel(parent), serv(serv) {
        dogs = serv.getServiceDogs();
    }
    ...
    QVariant data(const QModelIndex& index, int role = Qt::DisplayRole) const override {
        ...
        if (index.row() >= dogs.size() || index.row() < 0)
            return QVariant();

        if (role == Qt::DisplayRole) {
            const auto& dog = dogs.at(index.row());
...
public:
    ...
    void shortcuts() {
        ...
        connect(undoShortcut, &QShortcut::activated, this, [=]() {
            int ret = serv.undo();
            if (ret == 0) {
                QMessageBox::warning(this, "Warning", "No more undos left!");
                return;
            }
            QMessageBox::information(this, "Success", "Undo successful!");
            });
        connect(redoShortcut, &QShortcut::activated, this, [=]() {
            int ret = serv.redo();
            if (ret == 0) {
                QMessageBox::warning(this, "Warning", "No more redos left!");
                return;
            }
            QMessageBox::information(this, "Success", "Redo successful!");
            });
    }
public slots:
    void question() {
    ...
    void admin_menu() {
        ...
        QPushButton* button6 = new QPushButton("Undo/Redo");
        QPushButton* button7 = new QPushButton("Log out");
        ...
        connect(button6, &QPushButton::clicked, this, &GUI::adm_op6);
        connect(button7, &QPushButton::clicked, this, &GUI::main_menu);
        shortcuts();
    ...
    void adm_op6() {
		...
		connect(button3, &QPushButton::clicked, this, &GUI::admin_menu);
        shortcuts();
    ...
    void user_op3() {
        AdoptionTableViewModel* model = new AdoptionTableViewModel(serv);
        QTableView* view = new QTableView();
        view->setModel(model);
        view->setMinimumWidth(408);
        view->setMinimumHeight(380);
        view->setWindowTitle("Adoption list");
        view->show();
    }
    void user_op4() {
        window->close();
